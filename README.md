...
<li>Добавлен REST-API и документация к нему (в разработке)</li>
</ul>
...
<br>
<code>/accounts permit=ADMIN</code> - возвращает все аккаунты. 
Недоступно ни одной существующей роли из соображений безопасности.
<br>
<code>/accounts/{idAccount} permit=ALL</code> - возвращает данные указанного аккаунта.
Доступ есть у всех.

<h4>Создание:</h4>
<code>Method=POST</code>
<br>
<code>/accounts role=ADMIN</code> - создает аккаунт в базе. 
В теле необходимо отправит json. Пока недоступно, поскольку соответствующий функционал
не был реализован.
